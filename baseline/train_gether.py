...
def main():
    ...
    if conf.wandb.use:
        wandb.login()
        if model_args.do_sweep:
            config_dict = {}
            config_dict.update(OmegaConf.to_container(conf, resolve=True))
    ...
    model_args, data_args, training_args = set_args_by_config(
        model_args, data_args, training_args, conf, model_args.do_sweep
    )
    ...
    if do_sweep:
        for field, value in conf.items():
            if not isinstance(field, dict):
                setattr(new_training_args, field, value)
        new_training_args.data_seed = new_training_args.seed
